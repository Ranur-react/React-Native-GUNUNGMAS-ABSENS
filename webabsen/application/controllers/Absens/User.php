...
class User extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->template->display('Absens/user/index', $data);
	}
	public function create()
	{
		$d['level'] = $this->Muser->level();
		$this->load->view('Absens/user/create', $d);
	}
	public function get_select()
	{
		...
		if ($level == '1' || $level == "") {
			...
			$result  = $this->Muser->get_karyawan();
			$label   = "Karyawan";
			$display = "block";
			$data    .= "<option value=''>-- Pilih Karyawan --</option>";
			foreach ($result as $ka) {
				$data .= "<option value='$ka[id_karyawan]'>$ka[nama_karyawan]</option>";
			}
		} 
		$json['data']    = $data;
		$json['label']   = $label;
	...
	public function edit()
	{
		...
		$d['level'] = $this->Muser->level();
		$this->load->view('Absens/user/edit', $d);
	}
